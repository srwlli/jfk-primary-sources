...
const incidents = [
  ...
  },
  {
    ...
    data: {
      ...
      location: "Dallas, Texas",
      icon: "group",
      summary: "Silvia Odio, a Cuban exile, claimed that three men visited her Dallas apartment in late September 1963, one of whom was introduced as 'Leon Oswald.' The visit allegedly occurred while Oswald was traveling to Mexico City, raising questions about his associations and timeline.",
      ...
      significance: "The Odio incident is one of the most troubling loose ends in the JFK assassination investigation. If true, it suggests Oswald had connections to anti-Castro Cuban exiles and was being introduced to exile groups just weeks before the assassination. The Warren Commission struggled to explain the incident, initially attempting to identify the visitors as other individuals. The House Select Committee on Assassinations later found Odio's testimony credible and the incident genuine, concluding that Oswald or an impersonator had visited her.",
      relatedPeople: [
        { name: "Lee Harvey Oswald", href: "/people/lee-harvey-oswald" },
...
async function seed() {
  console.log('Seeding incidents table...')

  const { data, error } = await supabase
    .from('incidents')
    .upsert(incidents, { onConflict: 'slug' })
    .select()
